...
type 
  ...
  TMinValue* = object
    first*: int
    last*: int
    file*: string # TODO: there must be a better way than storing the file name in every value...
    case kind*: TMinKind
  ...
  TMinError* = enum        ## enumeration that lists all errors that can occur
    errNone,               ## no error
...
proc parseMinValue*(p: var TMinParser): TMinValue =
  ...
  case p.token
  of tkString:
    result = TMinValue(kind: minString, strVal: p.a, first: p.bufpos-p.a.len, last: p.bufpos, file: p.filename)
    p.a = ""
    discard getToken(p)
  of tkInt:
    result = TMinValue(kind: minInt, intVal: parseBiggestInt(p.a), first: p.bufpos-p.a.len, last: p.bufpos, file: p.filename)
    discard getToken(p)
  of tkFloat:
    result = TMinValue(kind: minFloat, floatVal: parseFloat(p.a), first: p.bufpos-p.a.len, last: p.bufpos, file: p.filename)
    discard getToken(p)
  of tkBracketLe:
    ...
    eat(p, tkBracketRi)
    result = TMinValue(kind: minQuotation, qVal: q, first: p.bufpos-p.a.len, last: p.bufpos, file: p.filename)
  of tkSymbol:
    result = TMinValue(kind: minSymbol, symVal: p.a, first: p.bufpos-p.a.len, last: p.bufpos, file: p.filename)
    p.a = ""
    discard getToken(p)
  else:
    raiseUndefinedError(p, "Undefined value: '"&p.a&"'")

proc minim(p: var TMinParser) = 
  var val: TMinValue
  while p.token != tkEof: 
    try:
      val = p.parseMinValue
    except:
      p.error getCurrentExceptionMsg()
    if val.kind == minSymbol:
      if SYMBOLS.hasKey val.symVal:
        SYMBOLS[val.symVal](val) 
      else:
        p.error("Undefined symbol: '"&val.symVal&"'")
    else:
      STACK.add val
